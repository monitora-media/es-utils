 ...
 * This is the ASCII-folded variant (without diacritical marks).
 *
...
public class SlovenianStemmerASCIIFold {
  /*
   ...
   * without diacritical marks (ASCII-folded: c, s, z instead of č, š, ž)</p>
   */
  ...
  private int palatalize(char[] s, int len) {
    ...
    // Handle c, z palatalization (ASCII-folded versions)
    // c + i/e → k (ASCII-folded č)
    if (endsWith(s, len, "ci") || endsWith(s, len, "ce")) {
      s[len - 2] = 'k';
    }
    // z + i/e → g (ASCII-folded ž)
    else if (endsWith(s, len, "zi") || endsWith(s, len, "ze")) {
      s[len - 2] = 'g';
    }
    // Note: s (ASCII-folded š) does not palatalize in Slovenian
